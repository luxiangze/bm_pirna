# piRNA Scatter Plot Visualization
...
# Configuration
INPUT_FILE <- here("data/raw/smRNA-seq/DHX15_td_pirna_variation/fc_DHX15_vs_WT.tsv")
OUTPUT_DIR <- here("figures/DHX15_td_pirna_variation")
LABEL_FILE <- NULL  # Set to here("path/to/labels.list") to enable custom labeling

# RPM threshold (0 = no filtering)
RPM_THRESHOLD <- 0.05
LOG2FC_THRESHOLD <- 1
...
title_text <- sprintf("Expression level of piRNAs: %s vs %s", treated_name, control_name)
...
if (!is.null(LABEL_FILE) && file.exists(LABEL_FILE)) {
  # Load custom labels from file (one piRNA id per line)
  label_ids <- readLines(LABEL_FILE, warn = FALSE)
  label_ids <- trimws(label_ids)
  label_ids <- label_ids[label_ids != ""]
  labels_df <- data %>% filter(piRNA_ID %in% label_ids)
} else {
  # Auto-label top up/down regulated significant piRNAs
...
# Calculate breaks for color scale (5 evenly spaced values)
breaks_seq <- seq(-max_log2fc, max_log2fc, length.out = 5)

# Custom color gradient: Deep Blue-White-Deep Red
color_gradient <- scale_color_gradient2(
  low = "#00008B",
  mid = "white",
  high = "#8B0000",
  limits = c(-max_log2fc, max_log2fc),
  breaks = breaks_seq,
...
if (!is.null(labels_df) && nrow(labels_df) > 0) {
  p <- p +
    geom_text_repel(
      data = labels_df,
      aes(x = control_rpm_plot, y = treated_rpm_plot, label = piRNA_ID),
      size = 6,
      color = "black",
...
p <- p +
  ...
  # Labels
  labs(
    x = sprintf("%s RPM (log10)", control_name),
    y = sprintf("%s RPM (log10)", treated_name)
  ) +
