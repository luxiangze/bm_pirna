...
Run scripts from the repository root so paths like `data/...` and
...
To rebuild the *Bombyx mori* OrgDb from `data/bm_ncbi_gene_ontology.gaf` which download frome NCBI, run:
...
|--------|-------|--------|-------------|
| `00_build_orgdb.R` | `data/bm_ncbi_gene_ontology.gaf` | `data/org.Bmori.eg.db_<version>.tar.gz` and installed `org.Bmori.eg.db` | Build a custom *Bombyx mori* OrgDb from NCBI GAF annotations for GO enrichment |
...
| `04_deseq2_go_kegg_enrichment.R` | *Bombyx mori* | `data/rnadiff_results/tables/differential/condition_control_treated.deseq2.results.txt` | `data/output/enrichment_deseq2/`, `graphs/enrichment_deseq2/` | GO and KEGG enrichment for DESeq2 DEGs (`pvalue < 0.05`, `|log2FoldChange| > 1`) |
...
For *Bombyx mori* DESeq2 enrichment:
